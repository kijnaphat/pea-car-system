...
const CAR_IMAGE_VARIANTS = new Set(['thumb', 'card', 'hero'])

export function getStoredCarImageUrl(imagePath, variant = 'hero') {
  if (!imagePath) return null
  const encodedPath = imagePath.split('/').map(encodeURIComponent).join('/')
  const safeVariant = CAR_IMAGE_VARIANTS.has(variant) ? variant : 'hero'
  return `/api/car-image?path=${encodedPath}&variant=${safeVariant}`
}

export function getCarImage(car, variant = 'hero') {
  return getStoredCarImageUrl(car?.image_path, variant) || getLegacyCarImage(car || {})
}
