  ...
  return (
    <div className="kpn-screen kpn-home flex min-h-[calc(100dvh-84px)] flex-col bg-[#f8f3fa] font-sarabun pb-0 text-[#241c27] lg:min-h-screen lg:bg-[#f3edf5] lg:pb-10" style={{WebkitFontSmoothing:'antialiased'}}>
      ...
      {showWelcomeAnnouncement && (
        <div className="kpn-detail-overlay fixed inset-0 z-[500] flex items-center justify-center p-5" role="alertdialog" aria-modal="true" aria-labelledby="welcome-announcement-title">
          <div className="absolute inset-0 bg-[#080d1b]/80 backdrop-blur-[5px]" aria-hidden="true" />
          <section className="relative flex max-h-[calc(100dvh-40px)] w-full max-w-[360px] flex-col overflow-hidden rounded-[25px] border border-white/15 bg-white shadow-[0_24px_70px_rgba(0,0,0,.4)] animate-slideUp">
            <div className="relative shrink-0 overflow-hidden bg-[linear-gradient(145deg,#080d1b_0%,#111a32_58%,#3d4db0_100%)] px-4 pb-5 pt-4 text-white">
              ...
              <div className="relative flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2.5">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-[12px] border border-white/20 bg-white p-1.5"><img src="/pea_logo.png" alt="PEA" className="h-full w-full object-contain" /></span>
                  <div className="min-w-0"><p className="text-[8px] font-bold tracking-[.15em] text-[#c9ff48]">WELCOME TO</p><p className="truncate text-[14px] font-black tracking-[-.3px]">KPN SMART CAR</p></div>
                </div>
                <button type="button" onClick={dismissWelcomeAnnouncement} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/15 bg-white/10 text-white transition-transform active:scale-90" aria-label="ปิดประกาศ"><Icon icon="ph:x-bold" width="14" height="14" /></button>
              </div>

              <div className="relative mt-4 flex items-center gap-3">
                <div className="min-w-0 flex-1">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-[#c9ff48]/25 bg-[#c9ff48]/10 px-2 py-1 text-[8px] font-bold text-[#c9ff48]"><span className="h-1.5 w-1.5 rounded-full bg-[#c9ff48]" />ประกาศสำหรับผู้ใช้งาน</span>
                  <h2 id="welcome-announcement-title" className="mt-2.5 text-[23px] font-black leading-[1.08] tracking-[-.8px]">จัดการรถส่วนกลาง<br/><span className="text-[#9eb0ff]">ง่ายในหน้าเดียว</span></h2>
                  <p className="mt-2 text-[10px] leading-relaxed text-white/65">ตรวจสถานะรถ สแกน QR นำรถออก–คืนรถ และติดตามข้อมูลล่าสุด</p>
                </div>
                <div className="flex h-[74px] w-[74px] shrink-0 rotate-[-5deg] items-center justify-center rounded-[24px] bg-[#c9ff48] text-[#080d1b] shadow-[0_15px_36px_rgba(174,235,36,.25)]">
                  <Icon icon="ph:car-profile-duotone" width="46" height="46" />
                </div>
              </div>
            </div>

            <div className="min-h-0 flex-1 overflow-y-auto bg-[#f8f9fc] px-4 py-4">
              <p className="text-[10px] font-bold tracking-[.1em] text-[#737b8e]">สิ่งที่ทำได้ในระบบ</p>
              <div className="mt-2.5 grid grid-cols-3 gap-1.5">
                {[
                  ...
                  <div key={item.label} className="rounded-[14px] border border-[#e7eaf1] bg-white px-1.5 py-2.5 text-center">
                    <span className={`mx-auto flex h-8 w-8 items-center justify-center rounded-[10px] ${item.color}`}><Icon icon={item.icon} width="19" height="19" /></span>
                    <p className="mt-1.5 text-[9px] font-bold text-[#333a4b]">{item.label}</p>
                  </div>
                ))}
              </div>
              <div className="mt-3 flex items-start gap-2 rounded-[13px] border border-[#dfe4ef] bg-white px-3 py-2.5 text-[#60697c]">
                <Icon icon="ph:info-duotone" width="17" height="17" className="mt-0.5 shrink-0 text-[#5547f7]" />
                <p className="text-[9px] leading-relaxed">แสดงครั้งเดียวในแท็บนี้ และจะแสดงใหม่เมื่อเปิดเว็บไซต์ในแท็บใหม่</p>
              </div>
            </div>

            <div className="shrink-0 border-t border-[#e7eaf1] bg-white px-4 pb-[max(13px,env(safe-area-inset-bottom))] pt-2.5">
              <button type="button" onClick={dismissWelcomeAnnouncement} className="flex w-full items-center justify-center gap-2 rounded-[15px] bg-[#101522] py-3.5 text-[14px] font-bold text-white shadow-[0_8px_22px_rgba(16,21,34,.18)] transition-transform active:scale-[.98]"><Icon icon="ph:check-circle-duotone" width="19" height="19" className="text-[#c9ff48]" />รับทราบและเริ่มใช้งาน</button>
            </div>
          </section>
        </div>
      )}
