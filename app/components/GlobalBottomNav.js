      ...
      <div className="pea-glass-nav flex h-[60px] w-full max-w-[590px] items-center gap-1 rounded-full border border-white/75 p-1.5 shadow-[0_14px_34px_rgba(51,28,67,.2)] md:h-[68px] md:max-w-[760px] md:gap-2 md:p-2">
        {items.map(item => (
          <button key={item.path} onClick={() => router.push(item.path)}
            className={`relative flex h-full min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full px-2 text-[11px] font-extrabold transition-all duration-300 md:gap-2 md:px-5 md:text-sm ${item.active ? 'pea-nav-active' : 'pea-nav-inactive hover:bg-[#f4f4f5]'}`}>
